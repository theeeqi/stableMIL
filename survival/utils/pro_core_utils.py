...
class EarlyStopping:
    ...
    def __init__(self, patience=20, stop_epoch=50,use_cindex = True,verbose=False):
        """
        Args:
...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(patience=20, stop_epoch=50, use_cindex=True,verbose = True)
    else:
        early_stopping = None
    ...
    for epoch in range(args.max_epochs):
        if args.task_type == 'survival':
            ...
            stop ,val_loss_surv,val_loss,c_index= validate_survival(cur, epoch, model, val_loader, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir)
    ...
    results_val_dict, test_cindex = summary_survival(model, test_loader, args.n_classes)
    print('Val c-Index: {:.4f}'.format(val_cindex))
    print('Test c-Index: {:.4f}'.format(test_cindex))
    if writer:
        writer.add_scalar('Val c-Index',  val_cindex, 0)
        writer.add_scalar('Test c-Index', test_cindex, 0)
        writer.close()
    return results_val_dict, test_cindex,val_cindex
...
def train_loop_survival(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16):   
    ...
    optimizer.zero_grad()
    for batch_idx, (data_WSI,coords, label, event_time, c) in enumerate(loader):

        data_WSI = data_WSI.to(device)
        coords = coords.to(device)
        
        data_WSI = data_WSI.unsqueeze(dim = 0)
        ...
        hazards, S, Y_hat = model(data_WSI,coords) # return hazards, S, Y_hat, A_raw, results_dict
        ...
        if (batch_idx + 1) % 100 == 0:
            print('batch {}, loss: {:.4f}, label: {}, event_time: {:.4f}, risk: {:.4f}, bag_size: {}'.format(batch_idx, loss_value + loss_reg, label.item(), float(event_time), float(risk), data_WSI[0].size(0)))

        loss = loss / gc + loss_reg
        loss.backward()

        if ((batch_idx + 1) % gc) == 0 or ((batch_idx + 1) ==length):  
            optimizer.step()
            optimizer.zero_grad()
    ...
    # c_index = concordance_index(all_event_times, all_risk_scores, event_observed=all_censorships) 
...
def validate_survival(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    ...
    if early_stopping:
        assert results_dir
        early_stopping(epoch, val_loss_surv,c_index, model, ckpt_name=os.path.join(results_dir, "s_{}_checkpoint.pt".format(cur)))
...
def summary_survival(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI,coords,label, event_time, c) in enumerate(loader):
        ...
        with torch.no_grad():
            hazards, survival, Y_hat = model(data_WSI,coords)
